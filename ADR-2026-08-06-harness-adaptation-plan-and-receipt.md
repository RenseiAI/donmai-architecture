...
adds `execution-preflight-protected-mcp-config/v2` as a closed
...
authored plan field. The initial receipt and host acknowledgement remain
immutable when file content rotates. V1 bytes and semantics remain unchanged;
v2 release, activation, runtime readiness, and live acceptance require the
accepted ADR's staged evidence.
