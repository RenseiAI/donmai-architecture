...
- **`ADR-2026-08-07-execution-context-pool-and-placement-vocabulary.md`** — **Proposed** — and deliberately so: five product-owner rulings (R1–R5) are folded in and the decisions are settled, but this corpus requires the affected reference-doc edits to land in the *same* commit that flips the status, and they are not made yet. The ADR carries a § "To accept this ADR" naming every document the accepting commit must edit. **Building blocks, stated explicitly (R1):** an **execution context** is the unit of agent execution (wire/CLI noun `instance`); a **sandbox** is one *kind* of execution context — the ephemeral, provider-minted kind — and not a synonym for the unit; a **pool** is a single-provider named *source* of execution contexts, and explicitly **not** a bag of heterogeneous sandboxes, not a kind of execution context, not sized, and not the policy over pools; a **capacity profile** is a named policy over an ordered list of pools, and is the one place unlike capacity composes. **Reconciles with what the corpus already Accepted:** `ADR-2026-08-05` D2's `PlacementRef{kind: host|pool|sandbox|remote_peer, resolution: exact|claim_bound}` already dissolved the sandbox-vs-pool false choice (one axis read at two *times*), and harness-as-composition-input was already Accepted — so **the divergence is in the code, not the corpus**, and the fix is delivery. Applies `ADR-2026-08-03` D4 rule 1 ("one noun, one referent") to `pool`, which has four live referents — three of them in this corpus (`013:160` daemon-as-worker-pool, `003`/`011`'s warm workarea cache incl. `/api/daemon/pool/*`, and `capacity.poolMaxDiskGb`). **Decides:** pools stay **single-provider** (D6.1); the per-project execution route is **promoted to a named, org-authored, project-granted capacity profile** (D6.2 — a shape change with a migration, *not* a rename); **predictive burst is deferred** because the named trigger (token/credit exhaustion) has no meter anywhere in the model, with **failure-triggered routing-around** as the first iteration (D7); per-pool permission enforcement is **deferred** until the policy engine and org-admin surface mature, with a recorded exit condition (D8); **authors name pools, never hosts** — pool naming *is* the placement mechanism (D9); the "sandbox provider" ambiguity is named a defect whose refactor is authorized in principle but **gated on a sizing pass** that separates wording-only from schema/wire work (D10). **`harness` is retained deliberately**; an earlier assessment claiming it diverged from industry norms is formally withdrawn and is not reopenable. Explicit rename-vs-behaviour table. Amends `001`, `003`, `004`, `011`, `013`, `ADR-2026-06-01`, `ADR-2026-08-03` on acceptance. Cross-cutting; mirrored as stub in `rensei-architecture`.
...
| [`ADR-2026-08-07-execution-context-pool-and-placement-vocabulary.md`](ADR-2026-08-07-execution-context-pool-and-placement-vocabulary.md) | Proposed | shared | Execution context, pool, and placement: one noun per referent, and where unlike capacity composes |
