...
- **`ADR-2026-08-07-execution-context-pool-and-placement-vocabulary.md`** — **Proposed.** Settles the capacity nouns: the unit of execution is an **execution context** (`sandbox` narrows to the ephemeral, provider-minted kind); a **pool** is a named *source* of execution contexts, not a kind of one; and pool-versus-context is one placement axis at two *times* (`claim_bound` promise vs `exact` fact), reconciling `ADR-2026-08-05`'s sibling `PlacementRef.kind`s with the operator wire's containment view without editing either contract. Applies `ADR-2026-08-03` D4 rule 1 ("one noun, one referent") to `pool`, which has four live referents — three of them in this corpus (`013:160` daemon-as-worker-pool, `003`/`011`'s warm workarea cache incl. `/api/daemon/pool/*`, and `capacity.poolMaxDiskGb`). Records two positions never previously decided: pools stay **single-provider** (unlike capacity composes one level up, in the project's ordered pool list), and **there is no burst** — `004`'s burst row and the composing CLI's `cloud-burst` help string advertise a capability whose schema was deleted. Restates that harness is an authoring input, never derived from model identity (delivery of `ADR-2026-06-06` / `ADR-2026-08-05`, not new scope). Mostly rename and reconciliation; explicit rename-vs-behaviour table; five questions (Q1–Q5) deferred with stated defaults. Amends `001`, `003`, `004`, `011`, `013`, `ADR-2026-06-01`, `ADR-2026-08-03` on acceptance. Cross-cutting; mirrored as stub in `rensei-architecture`.
