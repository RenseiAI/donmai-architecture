...
5. **`~/.rensei/` paths** in `afcli/credentials` move to `.donmai/` (via the existing `statepath` helper); the one-release `.rensei` read-fallback already in `statepath` covers migration. → *Follow-on:* [`ADR-2026-06-03-injectable-state-dir.md`](ADR-2026-06-03-injectable-state-dir.md) refines this consolidation into an injectable seam so the closed composing binary can set its own brand dir (`~/.rensei/`); the OSS default stays `donmai`. That ADR is a refinement of this decision, not a reversal.
