...
**The execution-event spine is a shared durable contract.** Per
[`ADR-2026-08-19-durable-execution-event-bus-and-bounded-subscriptions.md`](ADR-2026-08-19-durable-execution-event-bus-and-bounded-subscriptions.md), normalized session, project, and workflow facts use one strict versioned envelope and one schema source. Source facts and outbox records commit before Redis, SSE, PTY, or message nudges; replay uses native exclusive positions, and bounded subscriptions admit visible workflow targets with deterministic durable receipts. Coordinator observation and agent-to-agent coordination remain separate stores: a nudge never copies an event body or advances an execution-event cursor. This is accepted architecture; implementation, migration, release, and activation remain separately gated.
