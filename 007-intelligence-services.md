...
> **AMENDED 2026-07-05 by `ADR-2026-07-05-self-referential-stdio-mcp-in-box-capability.md`:** the shipped **agent-facing delivery surface** for the Go Code Intelligence engine is a **self-referential in-box stdio MCP server**: the hidden `donmai mcp code-intel` subcommand (server name `af-code-intelligence`, six `af_code_*` tools, FQ prefix `mcp__af-code-intelligence__af_code_*`), spawned per session by the in-box runner via `os.Executable()` with an **explicit `--root <worktree>`** (never cwd), activated by the typed `codeIntel` block on QueuedWork. The legacy in-process `donmai_code_*` MCP tool names in "Contract: what kits and agents see" below describe the deprecated TS package only. That ADR fixes this as **the** delivery pattern for every in-box capability that must touch the sandbox working tree (`af_linear` next).
