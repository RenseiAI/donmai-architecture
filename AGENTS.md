...
- **`ADR-2026-08-07-execution-context-pool-and-placement-vocabulary.md`** — **Proposed; nothing to implement until Q1–Q5 are answered.** The capacity nouns: **execution context** is the unit (`sandbox` = the ephemeral kind only), a **pool** is a named *source* of execution contexts, and pool-vs-context is one placement axis at two times (`claim_bound` vs `exact`) — reconciling `ADR-2026-08-05` D2's sibling kinds with the operator wire's containment view with **no contract edit**. Applies `ADR-2026-08-03` D4 rule 1 to `pool` (four live referents, three in this corpus). Decides: pools stay single-provider (unlike capacity composes in the project's ordered pool list); **there is no burst** — `004`'s burst row and the composing CLI's `cloud-burst` help are false and get deleted. Restates harness-as-authoring-input. Read the rename-vs-behaviour table before treating any part of it as implementation work. Cross-cutting; mirrored as stub in `rensei-architecture`.
