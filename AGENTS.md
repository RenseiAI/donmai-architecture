...
- **`ADR-2026-08-07-execution-context-pool-and-placement-vocabulary.md`** — **Proposed; the decisions are settled, the reference-doc edits are not made yet.** Five product-owner rulings (R1–R5) are folded in; status stays `Proposed` only because this corpus requires the affected doc edits to land in the accepting commit — see its § "To accept this ADR" for the exact list. **The building blocks (R1):** an **execution context** is the unit (wire noun `instance`), a **sandbox** is one *kind* of it (ephemeral, provider-minted), a **pool** is a single-provider *source* of execution contexts (explicitly **not** a bag of heterogeneous sandboxes, no size, no policy of its own), and a **capacity profile** is a named policy over an ordered list of pools — the one place unlike capacity composes. Reconciles with the already-Accepted corpus: `ADR-2026-08-05` D2's `PlacementRef{kind, resolution}` already dissolved the sandbox-vs-pool false choice, and harness-as-composition-input is already Accepted — **the divergence is in the code, not the corpus**. Applies `ADR-2026-08-03` D4 rule 1 to `pool` (four live referents, three in this corpus). Decides: pools stay single-provider (D6.1) and the per-project route is **promoted to a named, org-authored, project-granted capacity profile** (D6.2 — a shape change, not a rename); **predictive burst is deferred for want of telemetry** and the first iteration is *failure-triggered routing-around* (D7); per-pool permission enforcement is deferred until the policy engine and admin surface mature, with a recorded exit condition (D8); **authors name pools, never hosts** (D9); the "sandbox provider" ambiguity is a defect and its refactor is authorized in principle but gated on a sizing pass (D10). `harness` is **retained deliberately** — a prior assessment claiming it diverged from industry norms is formally withdrawn. Read the rename-vs-behaviour table before treating any part of it as implementation work. Cross-cutting; mirrored as stub in `rensei-architecture`.
