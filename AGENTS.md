...
- **`ADR-2026-08-07-onboarding-is-the-only-user-action.md`** — **Proposed.** Makes "onboarding is the only moment a user is required to act" an architectural invariant: nine operational rules (D1-D9) for what "transparent" means — identity minted once and referenced not derived, machine state never derived from interactive human session state, scope changes as runtime operations rather than process-restart events, no remediation hint for platform-computable state, bounded-retry convergence with fail-closed paired to a signal, success asserted from post-conditions, conditions surfaced where the user is, one server-side operation per intent with client parity, and bidirectional desired-state convergence — plus an explicit **boundary of legitimate user action** (identity assertion, tenant-enrollment consent, user-held secrets, OS/device consent, irreversible actions) with three tests for classifying an interruption. Cross-cutting; mirrored as stub in `rensei-architecture`.
