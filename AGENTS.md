...
- **`ADR-2026-08-03-cli-noun-tree-fleet-retirement.md`** — **Proposed; nothing implemented.** Retires `fleet` as a top-level CLI noun and consolidates on two nouns (`host` = this machine, `capacity` = the org's execution capacity, reserved-but-unimplemented in OSS); gives the OSS binary a real `host` parent with `daemon` demoted to a hidden alias; fixes `fleet` to one single-machine meaning in OSS and schedules the legacy `fleet`/`worker` process-supervision subtree for removal; replaces the unfalsifiable "one release" alias promise with a declared-removal-version gate. Supersedes in part `ADR-2026-05-06-tui-noun-consolidation.md`; amends `011` and `014` on acceptance. Cross-cutting; mirrored as stub in `rensei-architecture`.
