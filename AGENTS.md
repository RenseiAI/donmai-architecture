...
- **`ADR-2026-07-18-bounded-terminal-workarea-leases.md`** — **Proposed; implementation pending and unreleased.** Provider-neutral terminal lease, local claim, acknowledgement/expiry release paths, separate acquisition quarantine, receiver-affine outbox, exact timing, and bounded idempotent provider release. Ownership ends only at durable `released`. Shared; a mirrored stub plus consumer protocol/sandbox/activation extension are required in `rensei-architecture` before acceptance. Amends `003`, `011`, and `013`.
