...
- **`ADR-2026-07-09-host-project-enablement-and-repository-resources.md`** — Separates service install, host project admission, and zero-to-many repository resources; adds explicit project identity to registration/session dispatch, one shared fair host limiter, truthful desired/applied status, and additive legacy migration. Cross-cutting; mirrored as a stub in `rensei-architecture`.
