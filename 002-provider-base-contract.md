...
`RequesterProvider` (added per ADR-2026-06-19) is the one **inbound** family: every other family is outbound (the engine calls out) or engine-initiated. It accepts an authenticated, structured request from a *registered external requester*, maps it onto a workflow dispatch (an `agent.request` trigger — `requester` is a deprecated read alias, see `016-workflow-engine.md`), and returns a structured response. It is the inbound dual of A2A and reuses the AgentRegistry `remote` protocol vocabulary (`'a2a' | 'mcp' | 'http'`) and the base contract's identity model (`authorIdentity` + signature). Its capability struct and response envelope:
