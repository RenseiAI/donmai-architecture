...
| **issue-tracker** | **NEXT — loader-bridge self-serve (W3, landed), freeze after** | Same shape as sandbox: working reference OSS impl, hardcoded-array registry. Converted to `register()` + per-provider `Manifest()` in W3 (B2); B3 then **folded the parallel webhook-event-normalisation adapter into the single session-scoped adapter** — webhook-event normalisation (`normalizeEvent`) + current-state reads (`fetchCurrentState`) now live on the one adapter resolved through the loader registry, so the family has exactly ONE adapter surface (no second registry to keep in sync). Freeze next. |
...
- W3-B3 completed the issue-tracker self-serve axis: the platform's former parallel webhook-event-normalisation adapter layer (a second per-provider registry duplicating the event→canonical-transition + current-state-read logic) was deleted, and its `normalizeEvent` / `fetchCurrentState` operations were migrated verbatim-equivalent onto the single session-scoped adapter resolved through the loader registry. The stage-trigger normaliser now resolves that one adapter via `register()`/`Manifest()`. Net: one adapter surface per tracker family, no dangling `{id,class}` provider strings, behaviour-preserving (event-normalisation + current-state outcomes unchanged, proven by the migrated per-tracker + cross-tracker regression tests).
